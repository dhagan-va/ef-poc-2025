...
namespace Edi837Ingester.Services
{
    public class S3EdiParserService(IS3Service s3Service, IEdiParserService ediParserService) : IS3EdiParserService
    {
        ...
        public async Task ParseFromS3Async(string bucketName, string fileName, ValidationLevel validationLevel)
        {
            ...
            await ediParserService.Parse(stream, validationLevel);
        }
    }
}
